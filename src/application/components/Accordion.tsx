    ...
    <div className="accordion-group accordion-group-hover mb-12 rounded-xl w-100 sm:w-3/4">
        {skills.map((skill, i) => {
            ...
            <div className="accordion shadow" key={i}>
                <input type="checkbox" id={`accordion-${i}`} className="accordion-toggle" />
                <label htmlFor={`accordion-${i}`} className="accordion-title">{skill.title}</label>
                <i className="accordion-icon"><FaChevronLeft /></i>
